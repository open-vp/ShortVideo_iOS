...
- (void)viewDidLoad {
    ...
    _videoPreview = [[UIView alloc] initWithFrame:CGRectMake(0, 64, self.view.width, 200 * kScaleY)];
...
//    _videoTitleField.placeholder = [NSString stringWithFormat:@"当前视频码率：%d",[TXVideoInfoReader getVideoInfo:_videoPath].bitrate];
    ...
    _videoPublish = [[TXUGCPublish alloc] initWithUserID:@""];
...
-(void) onPublishComplete:(TXPublishResult*)result
{
    if(!_cancelPublish){
        if(result.retCode == 0){
            //同步给业务server
            NSURL *videoUrl = [NSURL URLWithString:result.videoURL];
