...
- (void)onNetStatus:(NSDictionary *)param {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        NSLog(@"mem %lld", get_app_consumed_memory_bytes());
    });
}
...
//    self.view.backgroundColor = UIColor.blackColor;
    ...
    self.txtRtmpUrl = [[UITextField alloc] initWithFrame:CGRectMake(10, 30 + ICON_SIZE + 10, size.width - 25 - ICON_SIZE * 2 - 15, ICON_SIZE)];
    ...
    btnScan.frame = CGRectMake(size.width - 20 - ICON_SIZE * 2, 30 + ICON_SIZE + 10, ICON_SIZE, ICON_SIZE);
    ...
    btnNewUrl.frame = CGRectMake(size.width - 10 - ICON_SIZE, 30 + ICON_SIZE + 10, ICON_SIZE, ICON_SIZE);
    [btnNewUrl setImage:[UIImage imageNamed:@"new"] forState:UIControlStateNormal];
    [btnNewUrl addTarget:self action:@selector(clickNewUrl:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:btnNewUrl];
...
- (void)clickNewUrl:(UIButton*)btn
{
    ...
    [TCHttpUtil asyncSendHttpRequest:@"get_test_pushurl" httpServerAddr:kHttpServerAddr HTTPMethod:@"GET" param:nil handler:^(int result, NSDictionary *resultDict) {
        ...
        else
        {
            ...
            weakSelf.txtRtmpUrl.text = pusherUrl;
